...
REMOTE_SCHEMES = ("s3://", "s3a://", "s3n://")


def resolve_warehouse_dir(override: Union[str, Path, None] = None) -> str:
    """Resolve the warehouse directory from an explicit override or env var."""
    if override is not None:
        return _normalize_warehouse_path(override)

    env_override = os.environ.get("ICEBERG_WAREHOUSE_DIR")
    if env_override:
        return _normalize_warehouse_path(env_override)

    return str(DEFAULT_WAREHOUSE_DIR)
...
def _normalize_warehouse_path(value: Union[str, Path]) -> str:
    if isinstance(value, Path):
        return str(value.expanduser().resolve())

    value_str = str(value).strip()
    if value_str.startswith(REMOTE_SCHEMES):
        return value_str.rstrip("/")

    return str(Path(value_str).expanduser().resolve())
...
def build_spark_session(warehouse_dir: str, runtime_jar: Path) -> SparkSession:
    ...
    builder = SparkSession.builder.appName("iceberg-overwrite-demo")

    if not _is_remote_path(warehouse_path):
        ...
        if profile:
            builder = builder.config("spark.hadoop.fs.s3a.aws.credentials.profile", profile)
